...
import {
  ...
  LogIn, LogOut, Calendar, Settings, Shield, User, Clock, Layers
} from 'lucide-react';
...
export default function AuditLogs() {
  ...
  useEffect(() => { fetchLogs(); }, [companyId, categoryFilter, dateFilter]);
      ...
      <div className="space-y-5">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div className="section-header mb-0">
            ...
            <p className="section-subtitle">Histórico completo de atividades do seu negócio</p>
          </div>
          <Button variant="outline" size="sm" onClick={fetchLogs} className="self-start sm:self-auto gap-1.5">
        ...
        <div className="flex flex-col sm:flex-row gap-2">
          <div className="relative flex-1 sm:max-w-xs">
            ...
            <Input
              ...
              placeholder="Buscar ação, e-mail..."
          ...
          <div className="flex gap-2">
            ...
            />
            {dateFilter && (
              <Button variant="ghost" size="sm" onClick={() => setDateFilter('')} className="h-9 px-2 text-xs">✕</Button>
            )}
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {[
            { label: 'Total', value: filteredLogs.length, icon: ScrollText, color: 'text-primary' },
            ...
            { label: 'LGPD', value: filteredLogs.filter(l => l.category === 'lgpd').length, icon: Shield, color: 'text-red-600' },
          ].map(s => (
        ...
        {loading ? (
          ...
          <Card className="glass-card-static rounded-2xl">
            <CardContent className="flex flex-col items-center justify-center py-16 px-4">
              ...
              <p className="text-xs text-muted-foreground/70 mt-1">As atividades serão registradas automaticamente</p>
            </CardContent>
          </Card>
        ) : (
